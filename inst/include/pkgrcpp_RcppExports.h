...
namespace pkgrcpp {
    ...
    inline double dq_approx_pi(const int N) {
        typedef SEXP(*Ptr_dq_approx_pi)(SEXP);
        static Ptr_dq_approx_pi p_dq_approx_pi = NULL;
        if (p_dq_approx_pi == NULL) {
            validateSignature("double(*dq_approx_pi)(const int)");
            p_dq_approx_pi = (Ptr_dq_approx_pi)R_GetCCallable("pkgrcpp", "_pkgrcpp_dq_approx_pi");
        }
        RObject rcpp_result_gen;
        {
            RNGScope RCPP_rngScope_gen;
            rcpp_result_gen = p_dq_approx_pi(Shield<SEXP>(Rcpp::wrap(N)));
        }
        if (rcpp_result_gen.inherits("interrupted-error"))
            throw Rcpp::internal::InterruptedException();
        if (Rcpp::internal::isLongjumpSentinel(rcpp_result_gen))
